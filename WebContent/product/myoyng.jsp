...
<div class="s-div">
		<dl class="ss-dl">  
		<dt class="fo_noto" style="color:rgba(148,148,148); ">미용 기기</dt>
  ...
  <section id = "listForm">
   ...
   <h5 style="text-align: center; margin-top: 40px; margin-bottom: 40px;"><strong style="font-size: 50px; color: rgb(148,148,148); ">Beauty Device</strong></h5>
	<c:if test="${product_myList != null }"> <!--1. 상품목록이 있으면 -->
			<table class="productListTBL">
				<tr>
					<c:forEach var="product_my" items="${product_myList}" varStatus="i">
					<td class="productListTD">
						<div class="carbox-bd">
						<a href="product_my.daol?product_no=${product_my.product_no}"><img src="images/${product_my.product_image}" id="productImage"/></a>
						상품명 : ${product_my.product_name }<br/>
						용량 : ${product_my.product_cap }<br/>
						</div>
					</td>
...
<nav aria-label="Page navigation example" style="margin: 20px 0 20px 0;">
  ...
  <%if(productPageNum ==1 ) { %>
   <li class="page-item" >
    ...
    <li class="page-item">
      <a class="page-link" href="product_my.daol?productPageNum=${productPageNum-1}" aria-label="Previous">
        <span aria-hidden="true" style="color: 	#DC143C;">&laquo;</span>
      </a>
    </li>
    <%} %>
    <c:forEach var="i" begin="1" end="${MaxProductPageNum }" step="1" >
   		<li class="page-item" ><a class="page-link" href="product_my.daol?productPageNum=${i}" style="color: black;">${i}</a> </li>
	</c:forEach>
	<%if(productPageNum < MaxProductPageNum){%>
     <li class="page-item">
      <a  class="page-link" href="product_my.daol?productPageNum=${productPageNum+1}" aria-label="Next">

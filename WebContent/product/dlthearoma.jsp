...
<%@ include file="../header_nav3.jsp" %>
  ...
  <img class="top-img" src="images/메인_아로마.jpg" style="background: black;">
  <!----------------------------------------------------------------------->
<div style="text-align:center; padding: 0px; border-bottom: 1px solid rgba(153,112,0); " >
...
<img class="senter-img" src="images/메인_아로마.jpg" >
</div>

<div class="s-div">
		<dl class="ss-dl">  
		<dt class="fo_noto" style="color:rgb(153,112,0); ">D.L the aroma</dt>
		<dd>디엘더아로마는 불가리안산 퓨어 원료로 최고의 품질과 독일 정통 최상급 블랜딩 기술로 탄생한 하이퀄리티 아로마 전문 브랜드 입니다.
		</dd>
 ...
 <h5 style="text-align: center; margin-top: 40px; margin-bottom: 40px;"><strong style="font-size: 50px; color: rgba(153,112,0);">D.L THE AROMA</strong></h5>
	<c:if test="${product_deList != null }"> <!--1. 상품목록이 있으면 -->
			<table class="productListTBL">
				<tr>
					<c:forEach var="product_de" items="${product_deList}" varStatus="i">
					<td class="productListTD">
						<div class="carbox-de">
						<a style="padding : 60px;" href="product_deView.daol?product_no=${product_de.product_no}"><img src="images/${product_de.product_image}" id="productImage"/></a>
						<hr>
						<strong>${product_de.product_name }</strong><br/>
						<strong>${product_de.product_cap }</strong><br/>
						</div>
					</td>
	...
	<c:if test="${product_deList == null }"> <!--2. 상품목록이 없으면 -->
...
<nav aria-label="Page navigation example" style="margin: 20px 0 20px 0;">
  ...
  <%if(productPageNum ==1 ) { %>
   <li class="page-item" >
    ...
    <li class="page-item">
      <a class="page-link" href="product_de.daol?productPageNum=${productPageNum-1}" aria-label="Previous">
        <span aria-hidden="true" style="color: 	#DC143C;">&laquo;</span>
      </a>
    </li>
    <%} %>
    <c:forEach var="i" begin="1" end="${MaxProductPageNum }" step="1" >
   		<li class="page-item" ><a class="page-link" href="product_de.daol?productPageNum=${i}" style="color: black;">${i}</a> </li>
	</c:forEach>
	<%if(productPageNum < MaxProductPageNum){%>
     <li class="page-item">
      <a  class="page-link" href="product_de.daol?productPageNum=${productPageNum+1}" aria-label="Next">

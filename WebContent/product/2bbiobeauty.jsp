...
<%@ include file="../header_nav3.jsp" %>
               <img class="top-img" src="images/메인_2B.jpg" style="background: black;">
                
                 <!----------------------------------------------------------------------->
 	<div style="text-align:center; padding: 0px; border-bottom: 1px solid rgba(2,111,52); " >
...
<img class="senter-img" src="images/메인_2B.jpg" >
</div>

<div class="s-div">
		<dl class="ss-dl">  
		<dt class="fo_noto" style="color:rgb(2,111,52); ">2B Bio Beauty</dt>
		<dd>첨단기술의 강력한 효력으로 세밀하게 만들어진 2B Bio Beauty®의 제품들은 나이나 피부타입, 
		생활 환경을 뛰어넘어 현대를 살아가는 남성과 여성의 요구를 충족시켜주고 있습니다. 
		2B Bio Beauty®의 피부전문가들이 고품질의 피부관리 프로그램으로 놀라운 결과를 보장해 드립니다.
		</dd>
 ...
 <section id = "listForm">
   ...
   <h5 style="text-align: center; margin-top: 40px; margin-bottom: 40px;"><strong style="font-size: 50px; color: rgba(2,111,52); ">2B BIO BEAUTY</strong></h5>
	<c:if test="${product_2BList != null }"> <!--1. 상품목록이 있으면 -->
			<table class="productListTBL">
				<tr>
					<c:forEach var="product_2B" items="${product_2BList}" varStatus="i">
					<td class="productListTD">
						<div class="carbox-2B">
						<a style="padding : 60px;" href="product_2BView.daol?product_no=${product_2B.product_no}"><img src="images/${product_2B.product_image}" id="productImage"/></a>
						<hr>
								<strong style="margin-left: 5px;">${product_2B.product_name }</strong><br/>
								<strong style="margin-left: 5px;">${product_2B.product_cap }</strong><br/>
						</div>
					</td>
	...
	<c:if test="${product_2BList == null }"> <!--2. 상품목록이 없으면 -->
...
<nav aria-label="Page navigation example" style="margin: 20px 0 20px 0;">
  ...
  <%if(productPageNum ==1 ) { %>
   <li class="page-item" >
    ...
    <li class="page-item">
      <a class="page-link" href="product_2B.daol?productPageNum=${productPageNum-1}" aria-label="Previous">
        <span aria-hidden="true" style="color: 	#DC143C;">&laquo;</span>
      </a>
    </li>
    <%} %>
    <c:forEach var="i" begin="1" end="${MaxProductPageNum }" step="1" >
   		<li class="page-item" ><a class="page-link" href="product_2B.daol?productPageNum=${i}" style="color: black;">${i}</a> </li>
	</c:forEach>
	<%if(productPageNum < MaxProductPageNum){%>
     <li class="page-item">
      <a  class="page-link" href="product_2B.daol?productPageNum=${productPageNum+1}" aria-label="Next">

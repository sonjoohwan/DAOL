...
<!--윗부분  --------------------------------------------------------------------------------------->
 ...
 <div class ="divide30"></div>  
   <h5 style="text-align: center; margin-top: 40px; margin-bottom: 40px;"><strong style="font-size: 50px; ">HERBALIST</strong></h5>
	<c:if test="${product_hbList != null }"> <!--1. 상품목록이 있으면 -->
			<table class="productListTBL">
				<tr>
					<c:forEach var="product_hb" items="${product_hbList}" varStatus="i">
					<td class="productListTD">
						<div class="carbox">
						<a style="padding : 60px;" href="product_hb.daol?product_no=${product_hb.product_no}"><img src="images/${product_hb.product_image}" id="productImage"/></a>
						<strong style="margin-left: 5px;">${product_hb.product_name }</strong><br/>
						<strong style="margin-left: 5px;"> ${product_hb.product_cap }</strong><br/>
						</div>
					</td>
	...
	<c:if test="${product_hbList == null }"> <!--2. 상품목록이 없으면 -->
...
<nav aria-label="Page navigation example">
  ...
  <%if(productPageNum ==1 ) { %>
   <li class="page-item" >
    ...
    <li class="page-item">
      <a class="page-link" href="product_hb.daol?productPageNum=${productPageNum-1}" aria-label="Previous">
        <span aria-hidden="true" style="color: 	#DC143C;">&laquo;</span>
      </a>
    </li>
    <%} %>
    <c:forEach var="i" begin="1" end="${MaxProductPageNum }" step="1" >
   		<li class="page-item" ><a class="page-link" href="product_hb.daol?productPageNum=${i}" style="color: #1E90FF;">${i}</a> </li>
	</c:forEach>
	<%if(productPageNum < MaxProductPageNum){%>
     <li class="page-item">
      <a  class="page-link" href="product_hb.daol?productPageNum=${productPageNum+1}" aria-label="Next">

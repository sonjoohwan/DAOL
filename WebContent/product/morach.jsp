...
<!--윗부분  --------------------------------------------------------------------------------------->
 <section id = "listForm">
	<c:if test="${product_morList != null }"> <!--1. 상품목록이 있으면 -->
			<table class="productListTBL">
				<tr>
					<c:forEach var="product_mor" items="${product_morList}" varStatus="i">
					<td class="productListTD">
						<div class="carbox">
						<a href="product_mor.daol?product_no=${product_mor.product_no}"><img src="images/${product_mor.product_image}" id="productImage"/></a>
						상품명 : ${product_mor.product_name }<br/>
						용량 : ${product_mor.product_cap }<br/>
						</div>
					</td>
	...
	<c:if test="${product_morList == null }"> <!--2. 상품목록이 없으면 -->
...
<nav aria-label="Page navigation example">
  ...
  <%if(productPageNum ==1 ) { %>
   <li class="page-item" >
    ...
    <li class="page-item">
      <a class="page-link" href="product_mor.daol?productPageNum=${productPageNum-1}" aria-label="Previous">
        <span aria-hidden="true" style="color: 	#DC143C;">&laquo;</span>
      </a>
    </li>
    <%} %>
    <c:forEach var="i" begin="1" end="${MaxProductPageNum }" step="1" >
   		<li class="page-item" ><a class="page-link" href="product_mor.daol?productPageNum=${i}" style="color: #1E90FF;">${i}</a> </li>
	</c:forEach>
	<%if(productPageNum < MaxProductPageNum){%>
     <li class="page-item">
      <a  class="page-link" href="product_mor.daol?productPageNum=${productPageNum+1}" aria-label="Next">
